  ...
  return (
    <Box sx={{ textAlign: 'center', padding: '20px' }}>
      ...
      {loading ? (
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 2, marginTop: '20px' }}>
          {images.map((url, index) => (
            ...
            >
              <img src={url} alt={`Invoice Image ${index + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </Box>
          ))}
        </Box>
      ) : (
      ...
      <Modal open={!!selectedImage} onClose={() => setSelectedImage(null)}>
        ...
        >
          {selectedImage && (
            <img src={selectedImage} alt="Selected Invoice" style={{ width: '100%', height: 'auto', maxHeight: '80vh' }} />
          )}
        </Box>
      </Modal>
    </Box>
